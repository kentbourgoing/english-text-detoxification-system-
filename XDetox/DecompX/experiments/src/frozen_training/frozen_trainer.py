...
class Trainer:
    ...
    def create_optimizer(self):
        ...
        if self.optimizer is None:
            ...
            optimizer_grouped_parameters = [
                {
                    "params": [p for n, p in opt_model.named_parameters() if n in decay_parameters and not should_freeze(n)],
                    "weight_decay": self.args.weight_decay,
                },
                {
                    "params": [p for n, p in opt_model.named_parameters() if n not in decay_parameters and not should_freeze(n)],
                    "weight_decay": 0.0,
                },
            ]
            logger.warn(f"Optimizing Params:\n{[n for n, p in opt_model.named_parameters() if not should_freeze(n)]}")
